...
Синтаксис `using` гарантирует, что ресурс будет освобождён корректно, а исходная ошибка не будет потеряна.
...
Если соединение закроется до заверешения работы с транзакцией, это может привести к ошибке. `using` автоматически обеспечивает правильный порядок.
