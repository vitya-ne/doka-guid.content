...
Создадим функцию использования ресурса. Обратите внимание, что нам не нужно явно вызывать метод экземпляра 'close()`:
...
Автоматическое освобождение ресурсов решает две проблемы:
...
- Возникновение исключений — если произойдёт ошибка, обычный код освобождения может не выполниться.

Посмотрим на пример функции работы с ресурсом:
...
Освобождение ресурса происходит в блоке `finally`, чтобы этот код выполнился в случае возникновения ошибки при использовании ресурса. Однако если ошибка произойдёт при выполнении функции `closeResource()`, то ресурс может остаться неосвобождённым, а исходная ошибка потеряна.

Синтаксис `using` гарантирует, что ресурс будет освобождён корректноЮ а исходная ошибка не будет потеряна.

При работе с несколькими ресурсами важно учитывать порядок их освобождения. Это может быть критично, если ресурсы зависят друг от друга. Автоматическое освобождение ресурсов гарантирует, что ресурсы будут освобождаться в обратном порядке их создания (как в стэке).

Например, при работе с базой данных:
...
// 1. tx[Symbol.dispose]()
// 2. conn[Symbol.dispose]()
```

Если бы соединение закрылось до заврешения работы с транзакцией, это могло бы привести к ошибке. `using` автоматически обеспечивает правильный порядок.
