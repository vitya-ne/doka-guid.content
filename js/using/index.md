...
`using` решает две ключевые проблемы:

- Забывчивость — разработчик может забыть добавить код для освобождения ресурса (например, закрыть соединение с базой данных).
- Исключения — если произойдёт ошибка, обычный код освобождения может не выполниться.
