...
Функция `fetch()` предназначена для выполнения [HTTP](/tools/http-protocol/) запросов. Раньше для этой цели использовался `XMLHttpRequest`, однако `fetch()` более гибкое и универсальное решение.

Гибкость интерфейса `fetch()` основана на использовании специальных объектов:

- Request, хранит информацию о запросе (URL, HTTP-метод, формат передаваемых данных, данные запроса и многое другое);
- Response, содержит данные о результате и методы обработки ответа.

`fetch()` возвращает `Promise`, что позволяет отделить логику обработки ответа от самого запроса.
