...
Функция `fetch()` предназначена для выполнения [HTTP](/tools/http-protocol/) запросов. Раньше для подобной работы использовался XMLHttpRequest, однако `fetch()` более гибкая и мощная альтернатива.
